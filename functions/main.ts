...
@func XMLNode.open(Text %selector) {
  $exists = "false"
  # Check if Xpath node exists
  $(%selector) {
    $exists = "true"
  }
  match($exists) {
    ...
    with("false") {
      log("--> The: " + %selector + " Element does not exist or is being selected incorrectly. It is also possible the element is being added after this code is executed.")
    }
    # Element exists
    else() {
      $(%selector) {
...
# Setup mtv zone
@func XMLNode.mtv_setup(Text %zone, Text %itemsdiv, Text %div, Text %item) {
  open(%zone) {
    add_class("mtv-zone")
  }
  open(%itemsdiv) {
